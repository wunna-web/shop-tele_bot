...
def main():
    ...
    WEBHOOK_URL = os.getenv("WEBHOOK_URL", "").strip()   # Render service URL
    ...
    if WEBHOOK_URL:
        path = f"webhook/{BOT_TOKEN}"   # secret-ish path
        full_webhook = WEBHOOK_URL.rstrip("/") + "/" + path

        print("Starting webhook:", full_webhook)

        app.run_webhook(
            listen="0.0.0.0",
            port=PORT,
            url_path=path,             # IMPORTANT: no leading slash here
            webhook_url=full_webhook,  # Telegram setWebhook target
            allowed_updates=Update.ALL_TYPES,
        )
    else:
        # ✅ Local test အတွက် WEBHOOK_URL မရှိရင် polling fallback
